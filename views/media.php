		...
		while ( $loop->have_posts() ) : $loop->the_post(); 
			...
			$item.='<div class="col-xs-2 col-sm-2 col-lg-2">';
				...
				if($inWidget){
					...
					$item.='<a class="btn-xs btn-danger" href="' . $this->urls->editor . '?media&output='.$output.'&id='. $bookmark->ID . '&action=deleteBookmark&app='.$bookmark_provider.'" style="padding: 0px 5px;position: absolute;top: 11px;right: 25px;font-weight: bold;">x</a>';
				}
...
echo'<div id="media_library">';
	...
	echo'<div class="col-xs-3 col-sm-2">';
		...
		echo'<ul class="nav nav-tabs tabs-left">';
			...
			echo'<li class="active"><a href="#image-library" data-toggle="tab">Image Library</a></li>';
			echo'<li><a href="#user-images" data-toggle="tab">My Images</a></li>';
			
			echo'<li class="gallery_type_title">Bookmarks</li>';
			echo'<li><a href="#user-payment-urls" data-toggle="tab">Payment Urls</a></li>';
				...
				if( !empty($_GET['app']) && !empty($this->dialog->bookmark) ){
					...
					foreach($apps as $app){
						...
						if( in_array('payment',$app->types) ){
							...
							echo'<li role="presentation"'.$active.'><a href="#user-payment'.$app->slug.'" aria-controls="user-payment'.$app->slug.'" role="tab" data-toggle="tab">'.strtoupper($app->name).'</a></li>';
					...
					echo'<div class="tab-content row" style="margin-top:20px;">';
						...
						foreach( $apps as $app ){
							...
							if( in_array('payment',$app->types) ){
								...
								echo'<div role="tabpanel" class="tab-pane'.$active.'" id="user-payment'.$app->slug.'">';
									...
									echo'<div class="panel panel-default" style="background:#efefef;border-left:1px solid #ddd;">';
										...
										if(!empty($options)){
											...
											echo '<form style="padding:10px;" target="_self" action="' . $save_url . '" class="saveBookmarkForm" method="post">';
